...
internal static class Program
{
    ...
    private class DatabaseRecord
    {
        public int index;
        public string Name;
        public DatabaseCategory parent = null;
        ...
        foreach (DatabaseCategory databaseCategory2 in _databaseCategories)
        {
            ...
            foreach (DatabaseRecord record in databaseCategory2.Records)
            {
                ...
                foreach (DatabaseAttribute attribute in record.Attributes)
                {
                    ...
                    if (attribute.AttributeType == AttributeType.RecordLink && record.parent != null)
                    {
                        var recordLink = attribute.Values.FirstOrDefault(x => x is RecordLink) as RecordLink;
                        if (recordLink != null)
                        {
                            if (recordLink.RecordIndex != -1 && recordLink.CategoryIndex != -1)
                            {
                                ...
                                DatabaseCategory hLinkCat = databaseRecord.parent;
                                if (hLinkCat != null)
                                {
                                    string linkCatName = hLinkCat.Name.Replace('/', '.'); ;
                                    keyDataCollection5["Data"] = linkCatName;
                                    //in schema files 
                                    //like Data=Missions.Levels
                                }
                            }
                        }
                    }
